...
export default function ATSScorer() {
  ...
  // qui polluent le score sans être de vraies compétences
  ...
  const ACTION_VERBS = [
    ...
    "accompagné","évalué","conseillé","géré","déployé","livré","atteint","dépassé",
  ];
  ...
  const analyzeLocal = (cvText, jdText) => {
    ...
    for (const kw of topJdKeywords) {
      // Détection importance avant matching → booste le poids des mots critiques
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const parsed = analyzeLocal(cv, jd);
      setResult(parsed);
      setActiveTab("score");
    } catch (e) {
      setError("Erreur : " + (e.stack || e.message || String(e)));
    } finally {
      setLoading(false);
    }
  };
        ...
        {!result ? (
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              <button
                onClick={analyze}
                disabled={loading || !cv.trim() || !jd.trim() || cvLoading}
                className="action-btn"
                style={{
                  ...
                  opacity: !cv.trim() || !jd.trim() || cvLoading ? 0.4 : 1,
